                                ...
                                <template x-for="(item , index) in calenderData">
                                    <template x-if="index < monthLength1">
                                        <span @click="selectDate(item)"
                                            :class="{ 'text-danger': item.isHoliday, 'active-today': item
                                                .isToday, 'before-day': item.status ===
                                                    'disabled', 'active-start': moment(item.valueEn, format).isSame(
                                                        startDate), 'active-end': moment(item.valueEn, format).isSame(
                                                    endDate), 'active-between': allDate.includes(item) }"
                                            x-text="item.status === 'hidden' ? '' : getDay(item.value)"></span>
                                    </template>
                                </template>
                    ...
                    if (!['disabled', 'hidden'].includes(obj.status)) {
                        if (this.startDate === null) {
                            ...
                            if (moment(obj.valueEn, this.format).isSame(this.startDate)) {
                                ...
                                this.allDate = this.calenderData.filter(item => moment(item.valueEn, this.format).isBetween(
